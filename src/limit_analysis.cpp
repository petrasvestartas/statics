...
Shell::Shell(std::array<Point, 3> arc_points, double thickness, double density,
             double horizontal_reaction, double eccentricity0, double eccentricity1,
             int divisions) {
    // Three point arc
    geo::Point p0(-4, 0, 0);
    ...
    Vector translation_vector =
        Vector(-static_cast<double>(er[1][0]), -static_cast<double>(er[1][1]),
               -static_cast<double>(er[1][2]));
    for (auto& polygon : polygons) {
    ...
    // Equilibrium equations: horizontal, hertical and moment equilibrium to find the vertical right
    // reaction force Vl + Vr = W -> Vl and Vr are unknown Hr = Hl -> Use the same for simplicity
    ...
    // std::cout << "Hl[0]: " << Hl[0] << " el[1][1]: " << el[1][1] << " W[1]: " << W[1] << " c[0]:
    // " << c[0] << " el[1][0]: " << el[1][0] << std::endl;

    //  // W[1] * c[0] = Vl * el[1][0] + Vr * er[1][0] -> will take Vr eccentricity as 0 to have one
    //  uknonw
