...
namespace geo{
    ...
    Point intersect(const std::pair<geo::Point, geo::Point>& s1, const std::pair<geo::Point, geo::Point>& s2, double tol) {
        ...
        if (std::abs(determinant) < tol) {
            ...
            throw std::runtime_error("Lines are parallel or almost parallel within the given tolerance.");
        } else {
    ...
    std::vector<std::pair<geo::Point, geo::Point>> offset_segments(const std::vector<geo::Point>& points, const double& distance) {
        ...
        for (size_t i = 0; i < points.size()-1; ++i) {
            ...
            geo::Point p1(points[i+1][0]+perp[0], points[i+1][1]+perp[1], points[i+1][2]+perp[2]);
            segments.push_back({p0, p1});
        }
        return segments;
    }

    std::vector<geo::Point> offset_polyline(const std::vector<geo::Point>& polyline, double distance, double tol) {
