...
namespace wood
{
    namespace xml
    {
        ...
        bool read_xml_numbers(std::vector<std::vector<double>> &numbers)
        {
            ...
            {
                (void)e;
                printf("read_wood::xml -> CPP Wrong property, probaby wrong path \n");
                return false;
            }
            return true;
        }
        ...
        bool read_xml_polylines(std::vector<std::vector<IK::Point_3>> &polylines, const bool &simple_case, const bool &remove_duplicates)
        {
            ...
            if (!file_exists_0(file_path))
            {
                printf("read_wood::xml -> read_wood::xml|read_xml_polylines|File does not exist \n");
                return false;
            }
            else
            {
            ...
            {
                (void)e;
                printf("nread_wood::xml -> |read_xml_polylines|CPP Wrong property \n");
                return false;
            }
            return true;
        }
            ...
            const bool &remove_duplicates)
        {
            ...
            if (input_insertion_vectors.size() > 0)
                if (input_insertion_vectors.size() != input_polyline_pairs.size() * 0.5)
                {
                    ...
                    printf("nread_wood::xml -> |read_xml_polylines|CPP insertion vectors are given, but count of them is not equal to polyline count \n");
                    return false;
                }
            ...
            if (input_JOINTS_TYPES.size() > 0)
                ...
                {
                    printf("nread_wood::xml -> |read_xml_polylines|CPP joint types are given, but count of them is not equal to polyline count \n");
                    std::cout << "\n"
            ...
            const bool &remove_duplicates)
        {
            ...
            if (!file_exists_0(file_path))
            {
                printf("read_wood::xml -> wood::xml|read_xml_polylines | File does not exist \n");
                return false;
            }
            else
            {
                ...
                for (boost::property_tree::ptree::value_type &v : tree.get_child(property_to_read))
                {
                    ...
                    if (v.first == "polyline" || v.first == "Polyline")
                    {
                        ...
                        for (boost::property_tree::ptree::value_type &point : v.second)
                        {
                            ...
                            double y = point.second.get<double>("y");
                            double z = point.second.get<double>("z");
                            // printf("read_wood::xml -> read_xml_polylines | x: %f y: %f z: %f \n", x, y, z);
                            //  IK::Point_3 p(x, y, z);
        ...
        bool write_xml_polylines(std::vector<std::vector<IK::Point_3>> &polylines, const bool &simple_case)
        {
            ...
            {
                (void)e;
                printf("write_wood::xml -> CPP Something went wrong, probaby wrong path \n");
                return false;
            }
            return true;
        }
        ...
        bool write_xml_polylines_and_types(std::vector<std::vector<std::vector<IK::Point_3>>> &polylines_tree, std::vector<std::vector<wood::cut::cut_type>> &types_tree, int id, bool simple_case)
        {
