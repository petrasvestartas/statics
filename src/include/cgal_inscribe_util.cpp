    ...
    namespace inscribe_util
    {
        ...
        namespace internal
        {
            ...
            bool orient_polyline_to_xy_and_back(CGAL_Polyline &polyline, CGAL::Aff_transformation_3<IK> &xform_to_xy, CGAL::Aff_transformation_3<IK> &xform_to_xy_inv)
            {
                ...
                IK::Point_3 center;
                IK::Plane_3 plane;
                cgal::polyline_util::get_fast_plane(polyline, center, plane);
        ...
        std::tuple<IK::Point_3, IK::Plane_3, double> get_polylabel(const std::vector<CGAL_Polyline> &polylines, double precision)
        {
            ...
            IK::Point_3 origin;
            IK::Plane_3 plane;
            cgal::polyline_util::get_fast_plane(polylines_copy[0], origin, plane);
            ...
            for (auto &polyline : polylines_copy)
                cgal::polyline_util::transform(polyline, xform_toXY);
        ...
        bool inscribe_rectangle_in_convex_polygon(const std::vector<CGAL_Polyline> &polylines, CGAL_Polyline &result, std::vector<IK::Point_3> &points, IK::Segment_3 &direction, double scale, double precision, double rectangle_division_distance)
        {
            ...
            for (auto &polyline : polylines_copy)
                for (int i = 0; i < polyline.size() - 1; i++)
                {
                    ...
                    cgal::vector_util::interpolate_points(polyline[i], polyline[i + 1], divisions, division_points, 2);
            ...
            if (rectangle_division_distance > wood::GLOBALS::DISTANCE)
            {
                ...
                for (int i = 0; i < rect.size() - 1; i++)
                {
                    ...
                    cgal::vector_util::interpolate_points(rect[i], rect[i + 1], divisions, division_points, 2);
            ...
            else if (rectangle_division_distance < -wood::GLOBALS::DISTANCE)
            {
                ...
                cgal::vector_util::interpolate_points(rect[0], rect[1], divisions_u, division_points_edge_0, 1);
                cgal::vector_util::interpolate_points(rect[3], rect[2], divisions_u, division_points_edge_2, 1);

                points.reserve((divisions_u + 2) * (divisions_v + 2));
                for (int i = 0; i < (divisions_u + 2); i++)
                {
                    std::vector<IK::Point_3> temp_divisions;
                    cgal::vector_util::interpolate_points(division_points_edge_0[i], division_points_edge_2[i], divisions_v, temp_divisions, 1);
