...
namespace geo{
     ...
     */
    std::array<int, 2> get_moment_sign(
        const Point &p, 
        const Vector &f, 
        bool cw_is_positive = true);
     ...
     * @return std::array<double, 2>
     */
    std::array<double, 2> get_moment_with_eccentricity(
        const Point &p, 
        const Vector &f, 
        const double &e, 
        const double& deg);
    ...
    /**
     * @brief Get the vertical reaction forces given two eccentricities of a frame and a horizontal force.
     ...
     */
    double global_equilibrium_of_frame(
        const Vector& F0x, 
        const double& e0, 
        const double& en);
     ...
     *       deg0 ∠
     */
    double local_equilibrium_of_block(
        const Point& p0,
        const double& e0, 
        const Vector& F0x, 
        const Vector& F0y, 
